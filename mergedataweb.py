...
def merge_data(lista_dfs_o_dict, keys_merge=None):
    ...
    def _merge_lista(lista_dfs):
        ...
        # df0 = pd.concat(lista_dfs,join='outer')
        # df0 = lista_dfs[0].append(lista_dfs[1:])
        # and type(lista_dfs[0].index.freq) is Day:
        # TODO COMPROBAR Act_datos en mismo día --> FALLA algo!
        if len(lista_dfs) == 2 and lista_dfs[0].index[-1] == lista_dfs[1].index[-1]:
            df0 = lista_dfs[0]
            df0.update(lista_dfs[1])
        elif (all([len(df_i) == 1 for df_i in lista_dfs])) or (type(lista_dfs[0].index.freq) is Day
                                                               and len(lista_dfs_o_dict) > 2):
    ...
    if type(lista_dfs_o_dict[0]) is dict:
        if keys_merge is None:
            keys_merge = lista_dfs_o_dict[0].keys()
        dict_merge = dict()
        for k in keys_merge:
            lista_k = sorted([d[k] for d in lista_dfs_o_dict], key=lambda item: item.index[0])
